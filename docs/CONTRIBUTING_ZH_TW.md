...
[English](./CONTRIBUTING_EN.md) | [中文（簡體）](./CONTRIBUTING.md) | **繁體中文**
...
lecithin 沿用了和 Folia 相同的 patch 系統，並依照修改的部分分成了兩個目錄：

- `lecithin-api` - 基於 `Folia-API` / `Paper-API` / `Spigot-API` / `Bukkit-API` 所做出的修改。
- `lecithin-server` - 針對 Minecraft 原版伺服器既有邏輯所做的修改。
...
3. 執行完成後，repository  的根目錄下應該會出現以下幾組目錄： `lecithin-api` 與 `lecithin-server`、`folia-api` 與 `folia-server` 以及 `paper-api` 與 `paper-server`（以下統稱為 `*-api` 與 `*-server`）；
...
- `lecithin-api` ：新增 API 的修改請放在這裡
- `lophine-api` ：針對 lophine-API 的修改請放在這個資料夾
- `folia-api`：針對 Folia API 的修改請放在這個資料夾
- `paper-api`：對於 Paper API / Spigot API / Bukkit API 的修改請放在這個資料夾

2. Server部分

- `lecithin-server` ：針對 Minecraft 原版伺服器的修改、以及新增的檔案，都請放在這個資料夾
- `lophine-server`：針對 lophine-Server 的修改請放在這個資料夾
...
1. 在 `lecithin-server/src/main/resources/assets/lophine/lang` 目錄下新增或修改對應語言的檔案，加入本地化的注解；
