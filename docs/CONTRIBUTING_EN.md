Contributing to Lecithin
=======================

**English** | [中文（简体）](./CONTRIBUTING.md) | [中文（繁體）](./CONTRIBUTING_ZH_TW.md)
...
Lecithin uses as the same patching system as Lophine,  
and has been divided into two directories for the purpose of modifying different parts of it:

- `lecithin-api` - Modifications to `Lophine-API` / `Folia-API` / `Paper-API` / `Spigot-API` / `Bukkit-API`.
- `lecithin-server` - Modifications to Minecraft Vanilla Server's source logic.
...
3. After performing the operation, the following directory pairs should exist in the root directory of the warehouse: `lecithin-api` and `lecithin-server` , `folia-api` and `folia-server` , `paper-api` and `paper-server` (Referred to `*-api` and `*-server` as below)
...
- `lecithin-api` : Modifications to the new API
...
- `lecithin-server` : Changes and new files to the Minecraft vanilla server should be made in this folder
...
- Only commits after the last commit of Folia will be considered as Lecithin patches.
...
2. Run `git commit -a --fixup <hash>` in your terminal to make a fix-up commit (PS: do not commit changes of lecithin-created files)
...
4. Run Gradle's task `fixupPaperApiFilePatches` to regenerate lecithin-created files to patches (PS: do not commit again before you run this task)
...
1. Create or modify the corresponding language file under the `lecithin-server/src/main/resources/assets/lecithin/lang` directory to add localized comments;
